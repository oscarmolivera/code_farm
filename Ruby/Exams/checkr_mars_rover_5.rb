...
class Rover
  ...
  def explore
    ...
    bounds_x = (0..@grid_size[0]).cover?(@coordinates[0])
    bounds_y = (0..@grid_size[1]).cover?(@coordinates[1])
    if bounds_x && bounds_y
      new_coordenates = @coordinates.map(&:to_s).join(' ')
      final_facing = @rover_facing.to_s
      'new position:' + new_coordenates + ' ' + final_facing
    else
      'Signal lost: Rover out of bounds'
    end
  end
