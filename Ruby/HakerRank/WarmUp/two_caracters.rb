...
As an example, consider the string abaacdabd. If you delete the character a, you
will be left with the string bcdbd. Now,removing the character c leaves you with
a valid string bdbd having a length of 4. Removing either b or d at any point
would not result in a valid string.
Given a string 's',convert it to the longest possible string 't' made up only of
alternating characters. Print the length of string on a new line.
If no string 't can be formed, print'0' instead.
=end


def two_caracters(str)
  return 0 if str.chars.uniq.size < 2

  com = [];  sum = []
  str.chars.uniq.each do |l|
    com << str.count(l)
  end
  com.combination(2).to_a.each do |su| 
    sum << su.reduce(:+)
  end
  sum.sort.reverse[0]
end
# 7
p two_caracters("aabbccc")
